...
from simple_srt_service.settings import SimpleSettings


PROTOCOL_PREFIX = "__SUBALIGN_VLLM_ASCEND__"


class PersistentAscendVllmWorker:
    """Own one resident qwen-asr/vLLM-Ascend model process."""

    def __init__(self, settings: SimpleSettings, project_root: Path) -> None:
    ...
    def log_path(self) -> Path:
        return self.settings.data_dir / "npu-vllm-worker.log"
    ...
    def current_status(self) -> str:
        if (
            ...
            self.status = "error"
            self.error = f"NPU vLLM Worker exited with code {self.process.returncode}"
        return self.status

    def _tail_log(self, length: int = 8000) -> str:
        try:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.status = "error"
            self.error = "NPU vLLM Worker 模型加载超时"
            self.stop(force=True)
            raise RuntimeError(f"{self.error}\n{self._tail_log()}") from exc

        if message.get("event") != "ready":
            self.status = "error"
            self.error = str(message.get("error") or "NPU vLLM Worker 启动失败")
            self.stop(force=True)
            raise RuntimeError(f"{self.error}\n{self._tail_log()}")
        self.status = "ready"
    ...
    def _read_stdout(self) -> None:
        ...
        self.responses.put(
            {
                "event": "worker_exited",
                "error": f"NPU vLLM Worker exited with code {self.process.poll()}",
            }
        )
    ...
    def run_job(
        ...
        with self._command_lock:
            if self.current_status != "ready" or self.process is None:
                raise RuntimeError(
                    f"NPU vLLM Worker 不可用：{self.error or self.current_status}"
                )
                ...
                if event == "progress" and message.get("job_id") == job_id:
                    if progress_callback:
                        progress_callback(
                            ...
                            str(message.get("stage", "NPU vLLM 正在对齐")),
                        )
                    continue
                if event == "completed" and message.get("job_id") == job_id:
                    return
                if event in {"job_error", "worker_exited"}:
                    error = str(message.get("error") or "NPU vLLM Worker 任务失败")
