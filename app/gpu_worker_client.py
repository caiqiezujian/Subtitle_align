...
class PersistentGpuWorker:
    ...
    def log_path(self) -> Path:
        return self.settings.data_dir / "gpu-worker.log"
    ...
    def current_status(self) -> str:
        if (
            ...
            self.status = "error"
            self.error = f"GPU Worker exited with code {self.process.returncode}"
        return self.status

    def _tail_log(self, length: int = 4000) -> str:
        try:
    ...
    def start(self) -> None:
        ...
        command = [
            sys.executable,
            "-u",
            str(self.project_root / "persistent_gpu_worker.py"),
        ...
        if self.settings.engine_flash_attention:
        ...
        try:
            message = self.responses.get(timeout=self.settings.engine_startup_timeout_seconds)
        except queue.Empty as exc:
            self.status = "error"
            self.error = "GPU Worker 模型加载超时"
            self.stop(force=True)
            raise RuntimeError(f"{self.error}\n{self._tail_log()}") from exc
        if message.get("event") != "ready":
            self.status = "error"
            self.error = str(message.get("error") or "GPU Worker 启动失败")
            self.stop(force=True)
            raise RuntimeError(f"{self.error}\n{self._tail_log()}")
        self.status = "ready"
    ...
    def _read_stdout(self) -> None:
        ...
        self.responses.put(
            {
                "event": "worker_exited",
                "error": f"GPU Worker exited with code {self.process.poll()}",
            }
        )
        ...
        with self._command_lock:
            ...
            while True:
                ...
                if event in {"job_error", "worker_exited"}:
                    error = str(message.get("error") or "GPU Worker 任务失败")
