...
from simple_srt_service_ascend.language import detect_language  # noqa: E402
from simple_srt_service_ascend.settings import settings  # noqa: E402
from simple_srt_service_ascend.worker_client import (  # noqa: E402
    PersistentAscendWorker,
)


LOGGER = logging.getLogger("simple_srt_service_ascend")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(name)s | %(message)s",
)

worker = PersistentAscendWorker(settings, PROJECT_ROOT)
...
async def lifespan(_: FastAPI):
    ...
    LOGGER.info(
        "Loading resident models with Transformers on %s "
        "(ASCEND_RT_VISIBLE_DEVICES=%s)",
        settings.npu_device,
        settings.npu_visible_devices,
    )
    await asyncio.to_thread(worker.start)
    LOGGER.info("Ascend SRT alignment service is ready")
    yield
    await asyncio.to_thread(worker.stop)


app = FastAPI(
    title="Ascend SRT Alignment Service",
    description="上传一个音视频和一个原文 SRT，直接返回在昇腾 NPU 上对齐后的 SRT。",
    version="1.0.0-ascend",
    lifespan=lifespan,
)
