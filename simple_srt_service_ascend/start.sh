...
if [[ ! -f "$SERVICE_DIR/config.yaml" ]]; then
  ...
  echo "已创建 simple_srt_service_ascend/config.yaml，请修改 NPU 和模型路径后重新启动。"
  exit 1
fi
...
cd "$PROJECT_ROOT"
python3 -m simple_srt_service_ascend.check_environment
exec python3 -m simple_srt_service_ascend.run
